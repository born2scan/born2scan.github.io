...
```
This involves a *XOR cypher*, but the challenge intro remember us to go and search the simplest way to solve it! <br>
...
```
Now we have to break the famous XOR cypher and to do so we can refer to [dcode](https://www.dcode.fr/xor-cipher) to try to bruteforce it. In fact among all the solution... we find it!
...
```
Finally we can absume that number 1 represents string "a" (which index in printable is 10) and so on. So we can convert all the found distances into a string obtaining the message to wrap into flag{}
```python
