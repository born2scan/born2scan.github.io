...
Next on: **feeding** the frames through the _Tesseract_ OCR engine, **waiting** for the process to finish, and then **_grepping_** through the output using the RegEx kindly given by the CTF's organizers:

```bash
$ for f in ./frames/frame_*.jpg; do echo "*** Processing $f"; tesseract "$f" "./ocr/${$(basename $f)%.jpg}.txt"; done
$ grep -P 'flag{[a-zA-Z0-9_-]{12,40}}' ./ocr/frame_*.txt
```
